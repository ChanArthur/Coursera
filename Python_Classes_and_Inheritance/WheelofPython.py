...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        Guess a letter, phrase, or type 'exit' or 'pass': """.format(self.name, self.prizeMoney, category,
                                                                     obscuredPhrase, ', '.join(sorted(guessed))))
        return user_input
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        list_with_vowel = [letter for letter in LETTERS if letter not in guessed]
        list_with_out_vowel = [letter for letter in list_with_vowel if letter not in VOWELS]
        if self.prizeMoney < VOWEL_COST:
            return list_with_out_vowel
        elif self.prizeMoney >= VOWEL_COST:
            return list_with_vowel

    def getMove(self, category, obscurePhrases, guessed):
        possible_letter = self.getPossibleLetters(guessed)
        if possible_letter == []:
            return 'pass'
        ...
        if coin_flip:
            ...
            for letter in possible_letter:
                if self.SORTED_FREQUENCIES.index(letter) > max_index:
                    max_index = self.SORTED_FREQUENCIES.index(letter)
            return self.SORTED_FREQUENCIES[max_index]
        else:
            return random.choice(possible_letter)
